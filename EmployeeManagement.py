...
class EMSApp:
    ...
    def build_ui(self):
        f = tk.LabelFrame(self.root, text="Employee Info", font=("Arial", 12, "bold"))
        f.grid(row=0, column=0, padx=10, pady=5)

        labels = ["ID", "Name", "Type", "DOJ (YYYY-MM-DD)", "End Date (YYYY-MM-DD)", "Extra1", "Extra2"]
        self.entries = {}
        for i, lbl in enumerate(labels):
            tk.Label(f, text=lbl, font=("Arial", 12)).grid(row=i, column=0, sticky="w", pady=2)
            if lbl == "Type":
                ent = ttk.Combobox(f, values=["FullTime", "PartTime", "Manager"], state="readonly", font=("Arial", 12))
            else:
                ent = tk.Entry(f, font=("Arial", 12))
            ent.grid(row=i, column=1, pady=2, padx=5)
            self.entries[lbl] = ent
        tk.Button(f, text="Add", command=self.add, font=("Arial", 12)).grid(row=7, column=0, pady=5)
        tk.Button(f, text="Remove", command=self.remove, font=("Arial", 12)).grid(row=7, column=1, pady=5)

        sf = tk.LabelFrame(self.root, text="Search / Display", font=("Arial", 12, "bold"))
        sf.grid(row=1, column=0, padx=10, pady=5)
        self.search = tk.Entry(sf, font=("Arial", 12))
        self.search.grid(row=0, column=1, padx=2)
        tk.Label(sf, text="Search ID/Name", font=("Arial", 12)).grid(row=0, column=0, sticky="w", padx=2)
        tk.Button(sf, text="Search", command=self.search_emp, font=("Arial", 12)).grid(row=0, column=2, padx=2)
        tk.Button(sf, text="Show All", command=self.show_all, font=("Arial", 12)).grid(row=1, column=0, pady=3)
        tk.Button(sf, text="Total Payroll", command=self.payroll, font=("Arial", 12)).grid(row=1, column=1, pady=3)

        self.out = tk.Text(self.root, width=140, height=30, font=("Consolas", 16))
